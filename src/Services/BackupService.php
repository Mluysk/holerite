...
final class BackupService
{
    ...
    public function generateConfigurationBackup(): string
    {
        ...
        $holidays = $this->fetchSingle('SELECT * FROM holiday_settings ORDER BY id ASC LIMIT 1');
        ...
        return $this->encode([
            ...
            'holiday_settings' => $this->formatHolidaySettingsForBackup($holidays),
        ]);
    }
    ...
    public function restoreDatabaseBackup(string $json): void
    {
        ...
        $allowedTables = ['companies', 'contribution_settings', 'holiday_settings', 'employees', 'payrolls', 'payroll_items', 'users', 'audit_logs'];
        $clearOrder = ['payroll_items', 'payrolls', 'employees', 'audit_logs', 'holiday_settings', 'companies', 'contribution_settings', 'users'];
        ...
        try {
            ...
            foreach (['companies', 'contribution_settings', 'holiday_settings', 'employees', 'payrolls', 'payroll_items', 'users', 'audit_logs'] as $table) {
                if (!in_array($table, $allowedTables, true)) {
                    continue;
                }
    ...
    public function restoreConfigurationBackup(string $json): void
    {
        ...
        $holidayData = $payload['holiday_settings'] ?? null;
        ...
        if (is_array($holidayData) && $holidayData !== []) {
            $filtered = $this->filterHolidaySettingsData($holidayData);
            ...
            if ($filtered !== []) {
                ...
                $sql = sprintf(
                    'INSERT INTO %s (%s) VALUES (%s) ON DUPLICATE KEY UPDATE %s',
                    $this->wrapIdentifier('holiday_settings'),
                    implode(', ', array_map([$this, 'wrapIdentifier'], $columns)),
                    implode(', ', $placeholders),
                    implode(', ', $updates)
                );
     ...
     * @return array<string, mixed>|null
     */
    private function formatHolidaySettingsForBackup(?array $row): ?array
    {
        if ($row === null) {
            return null;
        }
        ...
        return [
            'id' => isset($row['id']) ? (int) $row['id'] : 1,
            'include_national' => isset($row['include_national']) ? ((int) $row['include_national'] !== 0) : true,
            'include_optional' => isset($row['include_optional']) ? ((int) $row['include_optional'] !== 0) : true,
            'include_municipal' => isset($row['include_municipal']) ? ((int) $row['include_municipal'] !== 0) : true,
            'state' => isset($row['state']) ? strtoupper(trim((string) $row['state'])) : 'SP',
            'city' => isset($row['city']) ? trim((string) $row['city']) : '',
            'municipal_holidays' => $this->normalizeMunicipalHolidayList($row['municipal_holidays'] ?? []),
            'updated_at' => $row['updated_at'] ?? null,
        ];
    }
     ...
     * @return array<string, mixed>
     */
    private function filterHolidaySettingsData(array $data): array
    {
        $id = isset($data['id']) ? (int) $data['id'] : 1;
        $includeNational = $this->normalizeBoolean($data['include_national'] ?? true);
        $includeOptional = $this->normalizeBoolean($data['include_optional'] ?? true);
        $includeMunicipal = $this->normalizeBoolean($data['include_municipal'] ?? true);

        $state = isset($data['state']) ? strtoupper(preg_replace('/[^A-Za-z]/', '', (string) $data['state'])) : '';
        ...
        $municipal = $this->normalizeMunicipalHolidayList($data['municipal_holidays'] ?? []);
        $municipalJson = json_encode($municipal, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

        if ($municipalJson === false) {
            $municipalJson = '[]';
        }

        return [
            'id' => $id,
            'include_national' => $includeNational ? 1 : 0,
            'include_optional' => $includeOptional ? 1 : 0,
            'include_municipal' => $includeMunicipal ? 1 : 0,
            'state' => $state,
            'city' => $city,
            'municipal_holidays' => $municipalJson,
        ];
    }
     ...
     */
    private function normalizeMunicipalHolidayList(mixed $value): array
    {
        $records = $this->decodeHolidayField($value);
        $normalized = [];
        ...
        foreach ($records as $entry) {
            ...
            if (!checkdate($month, $day, $referenceYear)) {
                // garante validação mesmo quando 29/02 em ano não bissexto
     ...
     * @return array<int, mixed>
     */
    private function decodeHolidayField(mixed $value): array
    {
        if (is_string($value) && $value !== '') {
            $decoded = json_decode($value, true);
