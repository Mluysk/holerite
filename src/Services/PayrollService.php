...
final class PayrollService
{
    ...
    public function createPayroll(int $employeeId, array $data): Payroll
    {
        ...
        $grossSalary = $this->roundMoney($baseSalaryAmount + $totalAllowances);
     ...
     */
    private function resolveInstallments(array $data, float $grossSalary, float $netSalary): array
    {
        $grossRounded = $this->roundMoney(max(0.0, $grossSalary));
        $netRounded = $this->roundMoney(max(0.0, $netSalary));

        if ($grossRounded <= 0.0 || $netRounded <= 0.0) {
            return [0.0, 0.0];
        }
        ...
        if ($advanceAmount === 0.0 && $remainingAmount === 0.0) {
            if ($shouldSplit) {
                ...
                $remainingAmount = $netRounded;
            }

            return [$advanceAmount, $remainingAmount];
        }

        if ($advanceAmount > $netRounded) {
            return [$netRounded, 0.0];
        }

        if ($remainingAmount > $netRounded) {
            $remainingAmount = $netRounded;
        }
        ...
        if ($remainingAmount > 0.0 && $advanceAmount === 0.0) {
            $advanceAmount = $this->roundMoney(max(0.0, $netRounded - $remainingAmount));
        }

        $total = $this->roundMoney($advanceAmount + $remainingAmount);
        if (abs($total - $netRounded) > 0.01) {
            $remainingAmount = $this->roundMoney(max(0.0, $netRounded - $advanceAmount));
        }
