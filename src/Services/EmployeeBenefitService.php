...
final class EmployeeBenefitService
{
    ...
    private function calculateVacations(Employee $employee, DateTimeImmutable $asOf): array
    {
        ...
        $today = new DateTimeImmutable($asOf->format('Y-m-d'));
        ...
        for ($index = 1; $index <= 4; $index++) {
            ...
            $workedReferenceEnd = $acquisitionEnd;
            if ($termination !== null && $termination < $workedReferenceEnd) {
                $workedReferenceEnd = $termination;
            }
            if ($today < $workedReferenceEnd) {
                $workedReferenceEnd = $today;
            }

            $workedMonths = $this->countEligibleMonths($cycleStart, $workedReferenceEnd);
            ...
            if ($termination !== null && $termination <= $acquisitionEnd) {
                ...
                $monthsLabel = sprintf('%d/12 meses', $workedMonths);
                $status = $workedMonths > 0 ? sprintf('Em aquisição (%s)', $monthsLabel) : 'Em aquisição';
                ...
                if ($today >= $preEligibilityNotice && $today <= $acquisitionEnd) {
                    $notice = true;
                    $upcomingByIndex[$index] = [
                        'index' => $index,
                        'available_on' => $availableFrom,
            ...
            $cycles[] = [
                ...
                'status' => $status,
                'eligible' => $eligible,
                'suggested_reference' => $eligible ? $availableFrom->format('Y-m') : null,
                'notice' => $notice,
            ];
        ...
        foreach ($cycles as $cycleData) {
            $concessionEnd = $cycleData['concession_end'];
            if ($concessionEnd instanceof DateTimeImmutable && $today <= $concessionEnd) {
                $nextCycle = $cycleData;
                break;
            }
        }

        if ($nextCycle === null && $cycles !== []) {
            $nextCycle = $cycles[array_key_last($cycles)];
        }

        return [
            'cycles' => $cycles,
            'upcoming_notices' => array_values($upcomingByIndex),
