...
final class EmployeeBenefitService
{
    ...
    private function calculateVacations(Employee $employee, DateTimeImmutable $asOf): array
    {
        ...
        for ($index = 1; $index <= 4; $index++) {
            ...
            $availableFrom = $this->startOfNextMonth($acquisitionEnd);
            ...
            if ($termination !== null && $termination < $acquisitionEnd) {
                ...
                $eligible = $asOf >= $availableFrom;
                if ($eligible) {
                    $status = $asOf > $concessionEnd ? 'Período de gozo vencido' : 'Período disponível para gozo';
                } else {
                    $status = sprintf('Disponível a partir de %s', $this->monthLabel($availableFrom));
                }
            ...
            if (!$eligible) {
                if ($workedMonths >= 12) {
                    if ($asOf <= $availableFrom) {
                        $daysUntilAvailability = (int) $asOf->diff($availableFrom)->format('%a');
                        if ($daysUntilAvailability <= 30) {
                            $notice = true;
                            $upcomingNotices[] = [
                                'index' => $index,
        ...
        if ($nextCycleCandidate !== null) {
            $nextCycle = [
                ...
                'available_from' => $nextCycleCandidate['available_from'] ?? $this->startOfNextMonth($nextCycleCandidate['acquisition_end']),
                'notice_from' => $nextCycleCandidate['notice_from'] ?? $this->startOfNextMonth($nextCycleCandidate['acquisition_end'])->modify('-30 days'),
    ...
    private function startOfNextMonth(DateTimeImmutable $date): DateTimeImmutable
    {
