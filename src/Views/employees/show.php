                ...
                <div class="flash flash-warning" style="margin-bottom:1.5rem;">
                    ...
                    <ul class="muted" style="margin:0.5rem 0 0 1rem;">
                        <?php foreach ($vacationNotices as $notice): ?>
                            <?php $deadline = $notice['concession_deadline'] ?? null; ?>
                            <li>
                                Período aquisitivo #<?= (int) $notice['index']; ?> disponível a partir de <?= htmlspecialchars($notice['available_on']->format('d/m/Y')); ?><?php if ($deadline instanceof DateTimeImmutable): ?> — conceder até <?= htmlspecialchars($deadline->format('d/m/Y')); ?><?php endif; ?>.
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
