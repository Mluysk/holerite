...
<section>
    ...
    <div class="tab-container" data-default-tab="<?= htmlspecialchars($defaultTab); ?>">
        <div class="tab-nav">
            ...
            <?php endif; ?>
            <?php if (in_array('holidays', $availableTabs, true)): ?>
                <button type="button" class="tab-button<?php if ($defaultTab === 'holidays'): ?> active<?php endif; ?>" data-tab="holidays">Feriados</button>
            <?php endif; ?>
        ...
        <?php if (in_array('holidays', $availableTabs, true)): ?>
        <div class="tab-content<?php if ($defaultTab === 'holidays'): ?> active<?php endif; ?>" id="tab-holidays">
            <div class="card">
                <h3 style="margin-top:0;">Calendário de feriados</h3>
                <p class="muted" style="margin-top:0;">Defina quais categorias de feriados o sistema considera e cadastre as datas municipais da sua cidade.</p>

                <?php if (!empty($isAdmin)): ?>
                    <form method="post" action="?action=update_holiday_settings" class="holiday-form" data-holiday-form>
                        <div class="holiday-settings-grid">
                            <section class="holiday-settings-grid__column">
                                <h4>Fontes consideradas</h4>
                                <p class="muted">Ative as categorias que devem aparecer no calendário e nos relatórios.</p>

                                <label class="checkbox-pill">
                                    <input type="checkbox" name="include_national" value="1" <?php if ($holidaySettings->includeNational()): ?>checked<?php endif; ?>>
                                    <span>Incluir feriados nacionais</span>
                                </label>
                                <label class="checkbox-pill">
                                    <input type="checkbox" name="include_optional" value="1" <?php if ($holidaySettings->includeOptional()): ?>checked<?php endif; ?>>
                                    <span>Incluir pontos facultativos</span>
                                </label>
                                <label class="checkbox-pill">
                                    <input type="checkbox" name="include_municipal" value="1" <?php if ($holidaySettings->includeMunicipal()): ?>checked<?php endif; ?>>
                                    <span>Incluir feriados municipais</span>
                                </label>

                                <div class="grid" style="margin-top:1rem;">
                                    <div>
                                        <label for="holiday_state">Estado (UF)</label>
                                        <input type="text" name="state" id="holiday_state" value="<?= htmlspecialchars($holidaySettings->getState()); ?>" maxlength="2" placeholder="UF">
                                    </div>
                                    <div>
                                        <label for="holiday_city">Cidade</label>
                                        <input type="text" name="city" id="holiday_city" value="<?= htmlspecialchars($holidaySettings->getCity()); ?>" placeholder="Ex: São Paulo">
                                    </div>
                                </div>
                            </section>
                            ...
                            <section class="holiday-settings-grid__column">
                                ...
                                <p class="muted">Cadastre feriados fixos da cidade. Eles serão aplicados todos os anos.</p>
                                <?php
                                $municipalRows = $holidaySettings->getMunicipalHolidays();
                                if ($municipalRows === []) {
                                    $municipalRows[] = ['name' => '', 'month' => '', 'day' => ''];
                                }
                                ?>
                                <div class="municipal-list" data-municipal-container>
                                    <?php foreach ($municipalRows as $index => $holiday): ?>
                                        <div class="municipal-row" data-municipal-row>
                                            <div class="municipal-row__field">
                                                <label class="visually-hidden" for="municipal-name-<?= $index; ?>">Descrição</label>
                                                <input type="text" id="municipal-name-<?= $index; ?>" name="municipal[name][]" value="<?= htmlspecialchars($holiday['name'] ?? ''); ?>" placeholder="Nome do feriado" autocomplete="off">
                                            </div>
                                            <div class="municipal-row__field municipal-row__field--small">
                                                <label class="visually-hidden" for="municipal-month-<?= $index; ?>">Mês</label>
                                                <input type="number" id="municipal-month-<?= $index; ?>" name="municipal[month][]" min="1" max="12" value="<?= isset($holiday['month']) ? htmlspecialchars((string) $holiday['month']) : ''; ?>" placeholder="Mês">
                                            </div>
                                            <div class="municipal-row__field municipal-row__field--small">
                                                <label class="visually-hidden" for="municipal-day-<?= $index; ?>">Dia</label>
                                                <input type="number" id="municipal-day-<?= $index; ?>" name="municipal[day][]" min="1" max="31" value="<?= isset($holiday['day']) ? htmlspecialchars((string) $holiday['day']) : ''; ?>" placeholder="Dia">
                                            </div>
                                            <button type="button" class="button button-ghost" data-remove-municipal>
                                                <i class="bi bi-x-lg" aria-hidden="true"></i><span class="visually-hidden">Remover</span>
                                            </button>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                                <button type="button" class="button button-secondary" data-add-municipal>
                                    <i class="bi bi-plus-lg" aria-hidden="true"></i> Adicionar feriado
                                </button>
                            </section>
                        </div>

                        <div class="form-actions" style="margin-top:1.5rem;">
                            <button type="submit" class="button">Salvar configurações de feriados</button>
                        </div>
                    </form>
                <?php else: ?>
                    <p class="muted">Entre como administrador para ajustar as fontes e feriados municipais considerados pelo sistema.</p>
                <?php endif; ?>
            </div>

            <div class="card" style="margin-top:1.5rem;">
                <h3 style="margin-top:0;">Próximos feriados</h3>
                <p class="muted" style="margin-top:0;">Visualize rapidamente os feriados que impactam o calendário e os relatórios.</p>
                <?php
                $upcomingHolidays = array_values(array_filter($holidayPreview, static function (array $holiday) use ($today): bool {
                    return isset($holiday['date']) && $holiday['date'] instanceof DateTimeImmutable && $holiday['date'] >= $today;
                }));
                $upcomingHolidays = array_slice($upcomingHolidays, 0, 6);
                ?>
                <?php if ($upcomingHolidays === []): ?>
                    <p class="muted" style="margin-top:1rem;">Nenhum feriado cadastrado para os próximos meses.</p>
                <?php else: ?>
                    <ul class="holiday-preview-list">
                        <?php foreach ($upcomingHolidays as $holiday): ?>
                            ...
                            ?>
                            <li class="holiday-preview-list__item">
                                <span class="holiday-preview-list__date"><?= htmlspecialchars($holidayDate->format('d/m/Y')); ?></span>
                                <div class="holiday-preview-list__info">
                                    <strong><?= htmlspecialchars($holiday['name'] ?? 'Feriado'); ?></strong>
                                    <span class="holiday-preview-list__scope"><?= htmlspecialchars($scope); ?></span>
                                </div>
                            </li>
                        <?php endforeach; ?>
                    </ul>
