...
/** @var array{totalAmount: float, averageAmount: float, periodCount: int, highest: array|null, lowest: array|null, latest: array|null, earliest: array|null, payrollCount: int, employeeCount: int, valeTotals: array{manual: float, transport: float, combined: float}} $summary */
/** @var array<int, array{period: string, total: float, count: int, percentage: float}> $breakdown */
/** @var array<int, array{period: string, manual: float, transport: float, total: float}> $valeBreakdown */
...
$valeTotals = $summary['valeTotals'];
$valeManualTotal = $valeTotals['manual'];
$valeTransportTotal = $valeTotals['transport'];
$valeCombinedTotal = $valeTotals['combined'];
...
<section class="report">
    ...
    <article class="report__paper">
        ...
        <section class="report-summary" aria-label="Resumo do relatório">
            ...
            <div class="report-summary__card">
                <span>Vales de produtos</span>
                <strong>R$ <?= number_format($valeManualTotal, 2, ',', '.'); ?></strong>
            </div>
            <div class="report-summary__card">
                <span>Vale-transporte</span>
                <strong>R$ <?= number_format($valeTransportTotal, 2, ',', '.'); ?></strong>
            </div>
            <div class="report-summary__card">
                <span>Total de vales</span>
                <strong>R$ <?= number_format($valeCombinedTotal, 2, ',', '.'); ?></strong>
            </div>
        </section>
            ...
            <section class="report-details" aria-label="Contexto do relatório">
                ...
                <div class="report-details__column">
                    ...
                    <ul class="report-details__list">
                        ...
                        <li>
                            <strong>Vales de produtos descontados</strong>
                            <span>R$ <?= number_format($valeManualTotal, 2, ',', '.'); ?></span>
                        </li>
                        <li>
                            <strong>Vale-transporte descontado</strong>
                            <span>R$ <?= number_format($valeTransportTotal, 2, ',', '.'); ?></span>
                        </li>
                    </ul>
                </div>
            </section>
        ...
        <section class="report__table" aria-label="Pagamentos líquidos por período">
            ...
            <section class="report__table" aria-label="Descontos de vale e vale-transporte por período">
            <?php if ($valeBreakdown === []): ?>
                <p class="report__empty">Nenhum desconto de vale ou vale-transporte registrado neste escopo.</p>
            <?php else: ?>
                <div class="table-responsive">
                    <table>
                        <thead>
                        ...
                        <tr>
                            <th>Total geral</th>
                            <th class="text-right">R$ <?= number_format($valeManualTotal, 2, ',', '.'); ?></th>
                            <th class="text-right">R$ <?= number_format($valeTransportTotal, 2, ',', '.'); ?></th>
                            <th class="text-right">R$ <?= number_format($valeCombinedTotal, 2, ',', '.'); ?></th>
                        </tr>
                        </tfoot>
                    </table>
                </div>
