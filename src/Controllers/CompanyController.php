...
final class CompanyController extends Controller
{
    ...
    public function __construct(
        ...
        private HolidaySettingsRepository $holidayRepository,
    ...
    public function edit(): void
    {
        ...
        $allowedTabs = $isAdmin
            ? ['company', 'appearance', 'holidays', 'discounts', 'password', 'users', 'backups']
            : ['appearance', 'password'];
        ...
        $holidaySettings = $this->holidayRepository->get();
        $holidayPreview = $this->holidayService->getHolidaysForYear((int) (new DateTimeImmutable('now'))->format('Y'));
    ...
    public function update(array $data, array $files = []): void
    {
        ...
        try {
            ...
            $newLogo = $this->processHeaderLogoUpload($files['header_logo'] ?? null);

            if ($newLogo !== null && $newLogo !== $company->getHeaderLogoPath()) {
                $this->removeManagedLogo($company->getHeaderLogoPath());
                $updated->setHeaderLogoPath($newLogo);
            }

            $this->companyRepository->save($updated);
            $GLOBALS['holerite_company'] = $updated;
     ...
     * @param array<string, mixed> $data
     */
    public function updateHolidaySettings(array $data): void
    {
        $this->ensureAdmin();

        $settings = $this->holidayRepository->get();

        try {
            $settings->setIncludeNational($this->toBool($data['include_national'] ?? '1'));
            $settings->setIncludeOptional($this->toBool($data['include_optional'] ?? '1'));
            $settings->setIncludeMunicipal($this->toBool($data['include_municipal'] ?? '1'));

            $state = strtoupper(trim((string) ($data['state'] ?? $settings->getState())));
            if ($state !== '') {
                $settings->setState($state);
            }

            $city = trim((string) ($data['city'] ?? $settings->getCity()));
            $settings->setCity($city);

            $settings->setMunicipalHolidays($this->extractMunicipalInput($data['municipal'] ?? null));

            $this->holidayRepository->save($settings);
            $this->flash('success', 'Configurações de feriados atualizadas com sucesso.');
        } catch (Throwable $exception) {
            $this->flash('error', 'Não foi possível atualizar as configurações de feriados: ' . $exception->getMessage());
        }

        $this->redirect('?action=edit_company&tab=holidays');
    }
    ...
    public function updateBranding(array $data, array $files = []): void
    {
        ...
        try {
            if ($brandName !== '') {
                if (function_exists('mb_strlen') && function_exists('mb_substr')) {
                    if (mb_strlen($brandName) > 255) {
                        $brandName = mb_substr($brandName, 0, 255);
                    }
                } elseif (strlen($brandName) > 255) {
                    $brandName = substr($brandName, 0, 255);
                }
            }
            ...
            if ($newLogo !== null && $newLogo !== $company->getHeaderLogoPath()) {
                $this->removeManagedLogo($company->getHeaderLogoPath());
                $company->setHeaderLogoPath($newLogo);
            }

            $this->companyRepository->save($company);
            $GLOBALS['holerite_company'] = $company;

            $this->flash('success', 'Identidade visual atualizada com sucesso.');
        } catch (RuntimeException $exception) {
            $this->flash('error', $exception->getMessage());
        } catch (Throwable $exception) {
            $this->flash('error', 'Não foi possível atualizar a identidade visual: ' . $exception->getMessage());
        }
    ...
    private function buildContributionSettings(
        ...
        $manualAllowances = $this->parseManualAllowances(
            $data['manual_allowances'] ?? [],
            $current->getManualAllowances(),
            $defaults->getManualAllowances()
        );
     ...
     * @return string[]
     */
    private function parseManualAllowances($raw, array $current, array $defaults): array
    {
        $entries = [];
        ...
        if ($normalized === []) {
            $normalized = $current !== [] ? $current : $defaults;
        }
    ...
    private function fillCompany(Company $company, array $data): Company
    {
        ...
        $hasBrandName = array_key_exists('brand_name', $data);
        $brandName = $hasBrandName ? trim((string) $data['brand_name']) : $company->getBrandName();
     ...
     */
    private function processHeaderLogoUpload(?array $file): ?string
    {
        ...
        if ($error !== UPLOAD_ERR_OK) {
            throw new RuntimeException('Não foi possível enviar o novo logo. Tente novamente.');
        }

        $tmpName = isset($file['tmp_name']) ? (string) $file['tmp_name'] : '';

        if ($tmpName === '' || !is_uploaded_file($tmpName)) {
            throw new RuntimeException('O arquivo de logo enviado é inválido.');
        }

        $size = isset($file['size']) ? (int) $file['size'] : 0;
        if ($size > 2 * 1024 * 1024) {
            throw new RuntimeException('O logo do cabeçalho deve ter no máximo 2 MB.');
        }
        ...
        if (!is_string($mime) || !isset($allowed[$mime])) {
            throw new RuntimeException('Envie uma imagem PNG, JPG, WEBP ou SVG para o cabeçalho.');
        }
        ...
        try {
            ...
            $token = dechex(random_int(0, PHP_INT_MAX));
        }

        $filename = sprintf('header-logo-%s-%s.%s', date('YmdHis'), $token, $extension);
        ...
        if (!move_uploaded_file($tmpName, $destination)) {
            throw new RuntimeException('Não foi possível salvar o novo logo enviado.');
        }
     ...
     */
    private function extractMunicipalInput(mixed $value): array
    {
        if (!is_array($value)) {
            return [];
        }

        $names = isset($value['name']) && is_array($value['name']) ? $value['name'] : [];
        $months = isset($value['month']) && is_array($value['month']) ? $value['month'] : [];
        $days = isset($value['day']) && is_array($value['day']) ? $value['day'] : [];
        $count = max(count($names), count($months), count($days));
        $records = [];

        for ($index = 0; $index < $count; $index++) {
            $name = isset($names[$index]) ? trim((string) $names[$index]) : '';
            $month = isset($months[$index]) ? (int) $months[$index] : 0;
            $day = isset($days[$index]) ? (int) $days[$index] : 0;
    ...
    private function removeManagedLogo(string $path): void
    {
        $path = trim($path);
