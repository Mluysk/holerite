...
final class EmployeeController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $requestedTab = strtolower(trim((string) ($_GET['tab'] ?? 'active')));
        $activeTab = $requestedTab === 'terminated' ? 'terminated' : 'active';
    ...
    public function terminate(int $id, array $data): void
    {
        $employee = $this->repository->find($id);
        ...
        $today = new DateTimeImmutable('today');
        if (!$this->isActive($employee, $today)) {
            $this->flash('warning', 'Este colaborador já possui desligamento registrado.');
            $this->redirect('?action=list_employees&tab=terminated');
            return;
        }

        $rawDate = trim((string) ($data['termination_date'] ?? ''));

        if ($rawDate === '') {
            $terminationDate = $today;
        } else {
            try {
                $terminationDate = new DateTimeImmutable($rawDate);
            } catch (Throwable) {
                $this->flash('error', 'Data de desligamento inválida.');
                $this->redirect('?action=list_employees');
                return;
            }
        }
        ...
        try {
            ...
            $this->repository->update($employee);
            $this->flash('success', 'Desligamento registrado com sucesso.');
        } catch (Throwable $exception) {
            $this->flash('error', 'Não foi possível registrar o desligamento: ' . $exception->getMessage());
        }
