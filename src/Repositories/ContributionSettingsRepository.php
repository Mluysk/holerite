...
final class ContributionSettingsRepository
{
    ...
    public function save(ContributionSettings $settings): void
    {
        ...
        if ($payload['inss_brackets'] === false || $payload['irrf_brackets'] === false) {
            throw new RuntimeException('Não foi possível preparar as faixas de contribuição para salvamento.');
        }

        $sql = 'INSERT INTO contribution_settings (id, fgts_rate, inss_brackets, irrf_brackets) VALUES (:id, :fgts_rate, :inss_brackets, :irrf_brackets)
                ON DUPLICATE KEY UPDATE fgts_rate = VALUES(fgts_rate), inss_brackets = VALUES(inss_brackets), irrf_brackets = VALUES(irrf_brackets)';
