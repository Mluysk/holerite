(function () {
    ...
    function updateSummary() {
        ...
        if (transportCheckbox instanceof HTMLInputElement && transportCheckbox.checked) {
            transportDeduction = roundMoney(baseSalary * 0.06);
        }
        ...
        if (automaticDescriptions) {
            ...
            if (transportDeduction > 0) {
                valeLines.push(`Vale-transporte (6%): <strong>${formatter.format(transportDeduction)}</strong>`);
            }
            if (manualVale > 0) {
                valeLines.push(`Vales informados: <strong>${formatter.format(manualVale)}</strong>`);
            }
