(function () {
    ...
    function initHolidayForm(form) {
        if (!(form instanceof HTMLFormElement)) {
            return;
        }

        var container = form.querySelector('[data-municipal-container]');
        var addButton = form.querySelector('[data-add-municipal]');
        var stateInput = form.querySelector('#holiday_state');
        var municipalToggle = form.querySelector('input[name="include_municipal"]');
        var counter = container instanceof HTMLElement ? container.querySelectorAll('[data-municipal-row]').length : 0;

        function normalizeStateValue(input) {
            if (!(input instanceof HTMLInputElement)) {
                return;
            }
            var raw = input.value.replace(/[^A-Za-z]/g, '').toUpperCase().slice(0, 2);
            if (input.value !== raw) {
                input.value = raw;
            }
        }
        ...
        function addMunicipalRow(name, month, day) {
            ...
            row.className = 'municipal-row';
            row.setAttribute('data-municipal-row', '');

            var nameField = document.createElement('div');
            nameField.className = 'municipal-row__field';
            var nameInput = document.createElement('input');
            nameInput.type = 'text';
            nameInput.name = 'municipal[name][]';
            nameInput.placeholder = 'Nome do feriado';
            ...
            monthField.className = 'municipal-row__field municipal-row__field--small';
            var monthInput = document.createElement('input');
            monthInput.type = 'number';
            monthInput.name = 'municipal[month][]';
            monthInput.min = '1';
            monthInput.max = '12';
            monthInput.placeholder = 'Mês';
            monthInput.value = typeof month === 'number' || typeof month === 'string' ? month : '';
            monthField.appendChild(monthInput);
            row.appendChild(monthField);

            var dayField = document.createElement('div');
            dayField.className = 'municipal-row__field municipal-row__field--small';
            var dayInput = document.createElement('input');
            dayInput.type = 'number';
            dayInput.name = 'municipal[day][]';
            dayInput.min = '1';
            dayInput.max = '31';
            dayInput.placeholder = 'Dia';
            dayInput.value = typeof day === 'number' || typeof day === 'string' ? day : '';
            ...
            removeButton.setAttribute('data-remove-municipal', '');
            removeButton.innerHTML = '<i class="bi bi-x-lg" aria-hidden="true"></i><span class="visually-hidden">Remover</span>';
        ...
        function ensureRows() {
            ...
            var rows = target.querySelectorAll('[data-municipal-row]');
            if (rows.length === 0) {
                addMunicipalRow('', '', '');
            }
        }
        ...
        if (container instanceof HTMLElement) {
            container.addEventListener('click', function (event) {
                ...
                if (target.hasAttribute('data-remove-municipal')) {
                    var row = target.closest('[data-municipal-row]');
                ...
                var button = target.closest('[data-remove-municipal]');
                if (button instanceof HTMLElement) {
                    var rowNode = button.closest('[data-municipal-row]');
                    if (rowNode && rowNode.parentNode === container) {
                        container.removeChild(rowNode);
                        ensureRows();
                    }
                }
            });
        ...
        if (stateInput instanceof HTMLInputElement) {
            stateInput.addEventListener('input', function () {
                normalizeStateValue(stateInput);
            });
            normalizeStateValue(stateInput);
        }

        if (municipalToggle instanceof HTMLInputElement) {
            var municipalSection = form.querySelector('.municipal-list');
            var addSectionButton = form.querySelector('[data-add-municipal]');

            function updateMunicipalState() {
                if (!(municipalSection instanceof HTMLElement)) {
                    return;
                }
                municipalSection.classList.toggle('municipal-list--disabled', !municipalToggle.checked);
                if (addSectionButton instanceof HTMLButtonElement) {
                    addSectionButton.disabled = !municipalToggle.checked;
                }
            }

            municipalToggle.addEventListener('change', updateMunicipalState);
            updateMunicipalState();
        }
    ...
    onReady(function () {
        ...
        var holidayForm = document.querySelector('[data-holiday-form]');
        if (holidayForm) {
            initHolidayForm(holidayForm);
        }
