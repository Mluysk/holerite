...
$holidaySettingsRepository = new HolidaySettingsRepository();
...
$holidayService = new HolidayService($holidaySettingsRepository);
...
$dashboardController = new DashboardController($employeeRepository, $payrollRepository, $employeeBenefitService, $holidayService);
...
switch ($action) {
    ...
    case 'update_holiday_settings':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $companyController->updateHolidaySettings($_POST);
        } else {
            $companyController->edit();
        }
        break;
    ...
    case 'update_company':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $companyController->update($_POST, $_FILES ?? []);
        } else {
            $companyController->edit();
        }
        break;
    case 'update_branding':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $companyController->updateBranding($_POST, $_FILES ?? []);
        } else {
            $companyController->edit();
        }
        break;
    ...
    case 'terminate_employee':
        $id = (int) ($_GET['id'] ?? 0);
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $employeeController->terminate($id, $_POST);
        } else {
            $employeeController->index();
        }
        break;
